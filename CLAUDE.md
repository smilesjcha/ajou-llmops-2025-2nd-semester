...
- `python3.11 -m venv ajou-llmops` - Create virtual environment with Python 3.11
- `source ajou-llmops/bin/activate` (Linux/Mac) - Activate virtual environment
- `deactivate` - Deactivate virtual environment

### Poetry Package Management (Recommended)
...
- `poetry run pytest --cov=src` - Run tests with coverage report
...
- `python3 -m unittest` - Run tests with unittest

### Code Quality Commands
- `poetry run black .` - Format code with Black
- `poetry run black --check .` - Check code formatting without changes
- `poetry run isort .` - Sort imports
- `poetry run isort --check-only .` - Check import sorting
- `poetry run flake8` - Run linting with Flake8
- `poetry run mypy src/` - Run type checking with MyPy

### Development Tools
- `poetry show` - Show installed packages
- `poetry show --tree` - Show dependency tree
- `python3 -c "import sys; print(sys.version)"` - Check Python version
